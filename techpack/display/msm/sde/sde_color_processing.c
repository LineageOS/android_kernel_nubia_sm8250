...
#define FOD_PRESS_LS 1638842112
#define FOD_PRESS_EN 1853661824

bool sde_is_fod_pressed(struct drm_crtc *crtc)
{
	struct sde_crtc_state *cstate = to_sde_crtc_state(crtc->state);
	int val = sde_crtc_get_property(cstate, PLANE_PROP_FOD);

	return (val == FOD_PRESS_LS || val == FOD_PRESS_EN);
}
