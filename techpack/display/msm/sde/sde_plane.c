 ...
 * y = ((z * (sqrt(x) / sqrt(32768))) + ((100 - z) * (x / 32768)^2)) / 100
 * with z = 115
 *
 ...
 */
inline u32 csc_nonlin_conv(u64 pcc)
{
	const u8 mul_const = 15;
	const u8 div_const = 100;
	u64 res, sqrt, pow;

	sqrt = DIV_ROUND_CLOSEST_ULL(PREC_INDEX * int_sqrt(pcc * PREC_INDEX) * PREC_INDEX,
						int_sqrt(PCC_ONE * PREC_INDEX));
	pow = DIV_ROUND_CLOSEST_ULL(PREC_INDEX * pcc * pcc * PREC_INDEX, PCC_ONE * PCC_ONE);
 ...
 *          the one of base_pcc.
 */
inline u32 csc_filter_coeff_calc(u32 base_pcc, u32 new_pcc)
{
	u32 csc_coeff = PREC_INDEX;
	u32 new_csc = csc_nonlin_conv(new_pcc);
	u32 base_csc = csc_nonlin_conv(base_pcc);

	if (base_csc > 0 && new_csc > 0)
		csc_coeff = DIV_ROUND_CLOSEST_ULL(new_csc * PREC_INDEX, base_csc);
					  ...
					  const struct sde_csc_cfg *csc_cfg)
{
	...
	for (i = 0; i < 3; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			if (color_filter) {
				if (pcc > 0)
					csc_filter_coeff[ij] = csc_filter_coeff_calc(base_pcc, pcc);
				else
